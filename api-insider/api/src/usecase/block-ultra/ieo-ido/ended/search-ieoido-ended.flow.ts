...
export class SearchIeoIdoEndedFlow {
  ...
  async execute(key: string) {
    ...
    if (key) { 
      ...
      query.where('LOWER(key) LIKE :key', { key: `%${searchKey}%` }).limit(BusinessSearch.MAX_ITEM);
    } else {
      query.orderBy('start_date', 'DESC', "NULLS LAST");
    } 
    query.select(BusinessSearch.SELECT_IEO_IDO_ENDED_PROPS);
    const datas = await query.getRawMany();
    return datas;
  }
}

export default SearchIeoIdoEndedFlow;
